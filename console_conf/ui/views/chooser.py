...
class ChooserConfirmView(BaseView):
    ...
    def __init__(self, controller, selection):
        ...
        summary = [
            TableRow([Text("Action:"), Color.info_error(Text(selection.action.title))]),
            TableRow([Text("Using:"),
                      Text("System seed of device {} version {} from {}".format(
                          selection.system.model.display_name,
                          selection.system.label,
                          selection.system.brand.display_name
                      ))]),
            # TODO:UC20 big large note that action can wipe your data and eat
            # your kittens?
        ]
        rows = [
            Pile([Text("")]),
